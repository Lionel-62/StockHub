...
export default function CreateInvoicePage() {
  ...
  return (
    <div className="p-3 md:p-0 h-full flex flex-col space-y-4 print:block print:p-0 print:h-auto">
      {/* Header compact */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#0a192f] p-4 rounded-xl border border-slate-200 dark:border-[#1c3a66] shadow-sm print:hidden">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/factures">
            <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:bg-[#112240]">
              <ChevronLeft size={18} />
            </Button>
          </Link>
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">{editId ? "Modifier le document" : docType === "devis" ? "Créer un devis" : "Créer une facture"}</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">{docType === "devis" ? "Établissez une proposition commerciale claire pour votre client." : "Générez et envoyez instantanément."}</p>
          </div>
        </div>
        <div className="flex w-full md:w-auto items-center gap-3">
          {!editId && (
            <Button 
              ...
              className="flex-1 md:flex-none text-slate-600 dark:text-slate-300 bg-white dark:bg-[#0a192f] border-slate-200 dark:border-[#1c3a66] hover:bg-slate-50 dark:bg-[#06101e] transition-all"
            >
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0 print:block print:min-h-0">
        ...
        <div className="w-full lg:w-[45%] flex flex-col space-y-6 overflow-y-auto pr-2 custom-scrollbar pb-10 print:hidden">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Type de document & Infos</h2>
            </div>

            {/* Commutateur Facture / Devis */}
            <div className="bg-slate-100 dark:bg-[#112240] p-1 rounded-xl flex items-center gap-1 border border-slate-200 dark:border-[#1c3a66]">
              <button
                type="button"
                onClick={() => {
                  setDocType("facture");
                ...
                className={cn("flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2",
                  docType === "facture" ? "bg-white dark:bg-[#0a192f] text-slate-900 dark:text-white shadow-sm" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:text-white"
                )}
              >
                <span>📄 Facture</span>
              </button>
              <button
                ...
                className={cn("flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2",
                  docType === "devis" ? "bg-white dark:bg-[#0a192f] text-amber-900 shadow-sm" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:text-white"
                )}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5 relative z-20">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">{docType === "devis" ? "Destinataire du devis *" : "Client Facturé *"}</label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">{docType === "devis" ? "Numéro de Devis" : "Numéro de Facture"}</label>
                <input 
                  type="text" 
                  value={invoiceNumber}
                  onChange={(e) => setInvoiceNumber(e.target.value)}
                  className="w-full p-2.5 border border-slate-200 dark:border-[#1c3a66] rounded-lg text-sm bg-slate-50 dark:bg-[#06101e] focus:outline-none font-mono"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">Date d'émission *</label>
                <DatePicker 
                  value={issueDate}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">{docType === "devis" ? "Validité de l'offre *" : "Date d'échéance *"}</label>
                <DatePicker 
                  value={dueDate}
                  onChange={setDueDate}
                />
              ...
              <div className="space-y-1.5 md:col-span-2 mt-2">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">Application de la TVA *</label>
                <div className="flex bg-slate-100 dark:bg-[#112240] p-1.5 rounded-lg border border-slate-200 dark:border-[#1c3a66]">
                  <button 
                    type="button"
                    onClick={() => setApplyTax(true)}
                    className={`flex-1 py-2 text-sm font-semibold rounded-md transition-all ${applyTax ? "bg-white dark:bg-[#0a192f] text-[#0b213f] shadow-sm border border-slate-200 dark:border-[#1c3a66]" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200"}`}
                  >
                    Avec TVA (18%)
                  </button>
                  <button 
                    ...
                    className={`flex-1 py-2 text-sm font-semibold rounded-md transition-all ${!applyTax ? "bg-white dark:bg-[#0a192f] text-slate-900 dark:text-white shadow-sm border border-slate-200 dark:border-[#1c3a66]" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200"}`}
                  >
                    Sans TVA
                  </button>
                </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-[#1c3a66]">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Articles & Services</h2>
            </div>
            
            <div className="space-y-3">
              {lines.map((line, index) => (
                <div key={line.id} className="p-4 border border-slate-200 dark:border-[#1c3a66] rounded-xl bg-slate-50 dark:bg-[#06101e]/50 space-y-3 relative group">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Ligne {index + 1}</span>
                    <button 
                  ...
                  <div className="space-y-1.5 relative z-20">
                    <label className="text-xs font-medium text-slate-600 dark:text-slate-300">Produit / Service</label>
                    <CustomSelect
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-600 dark:text-slate-300">Quantité</label>
                      <input 
                        type="number" 
                        min="1"
                        placeholder="1"
                        ...
                        className="w-full p-2 border border-slate-200 dark:border-[#1c3a66] rounded-lg text-sm bg-white dark:bg-[#0a192f] focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-600 dark:text-slate-300">Prix unitaire (FCFA)</label>
                      <input 
                        type="number" 
                        min="0"
                        placeholder="0"
                  ...
                  <div className="pt-2 text-right">
                    <span className="text-xs text-slate-500 dark:text-slate-400 mr-2">Total ligne :</span>
            ...
            <Button 
              variant="outline" 
              onClick={addLine}
              className="w-full border-dashed border-2 border-slate-200 dark:border-[#1c3a66] text-slate-600 dark:text-slate-300 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50/50 bg-white dark:bg-[#0a192f]"
            >
        ...
        <div className="w-full lg:w-[55%] bg-slate-100 dark:bg-[#112240] rounded-xl p-2 sm:p-4 lg:p-8 flex justify-center overflow-y-auto custom-scrollbar border border-slate-200 dark:border-[#1c3a66] shadow-inner print:block print:w-full print:bg-white dark:bg-[#0a192f] print:border-none print:shadow-none print:p-0 print:overflow-visible">
          
          <div id="invoice-preview" className="w-full max-w-2xl transition-all">
            {docType === "devis" ? (
              /* MODÈLE DE DEVIS MODERNE & COMMERCIAL */
              <div className="bg-[#f2f6f3] w-full rounded-2xl shadow-sm p-5 sm:p-8 md:p-12 text-slate-800 dark:text-slate-100 flex flex-col relative border border-[#d2ded5] print:shadow-none print:border-none print:m-0 print:p-8 print:bg-[#f2f6f3] print:[print-color-adjust:exact]">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
                  <div>
                    ...
                    <div className="mt-4 space-y-1 text-xs sm:text-sm">
                      ...
                      <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm sm:text-base">{selectedClient?.name || "Nom du client"}</p>
                      <p className="text-slate-600 dark:text-slate-300">{selectedClient?.email || "email@client.com"}</p>
                      {selectedClient?.phone && <p className="text-slate-600 dark:text-slate-300">{selectedClient.phone}</p>}
                  ...
                  <div className="text-left sm:text-right space-y-1 text-xs sm:text-sm flex flex-col sm:items-end">
                    ...
                    <p className="text-[11px] font-mono text-slate-500 dark:text-slate-400 pt-1">Réf : {invoiceNumber}</p>
                  </div>
                </div>
                    ...
                    <tbody className="divide-y divide-[#11313d]/15">
                      {lines.map((line, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 dark:bg-[#06101e] transition-colors">
                          <td className="py-3 px-3 text-slate-800 dark:text-slate-100 font-medium">{line.description || <span className="text-slate-300 italic">Article sans nom</span>}</td>
                          <td className="py-3 px-3 text-center text-slate-700 dark:text-slate-200">{line.quantity}</td>
                          <td className="py-3 px-3 text-right text-slate-700 dark:text-slate-200"><span className="font-mono">{formatCurrency(typeof line.unitPrice === 'number' ? line.unitPrice : (parseFloat(line.unitPrice) || 0))}</span></td>
                          <td className="py-3 px-3 text-right font-semibold text-slate-900 dark:text-white"><span className="font-mono">{formatCurrency((typeof line.quantity === 'number' ? line.quantity : (parseFloat(line.quantity) || 0)) * (typeof line.unitPrice === 'number' ? line.unitPrice : (parseFloat(line.unitPrice) || 0)))}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 mb-8">
                  ...
                  <div className="order-1 sm:order-2 w-full sm:w-80 space-y-4 ml-auto">
                    ...
                    <div className="border border-[#11313d]/20 rounded-lg overflow-hidden bg-white dark:bg-[#0a192f] text-xs sm:text-sm shadow-2xs">
                      ...
                      <div className="flex justify-between py-2 px-3 border-b border-[#11313d]/10">
                        <span className="text-slate-600 dark:text-slate-300 font-medium">TVA ({applyTax ? "18%" : "0%"})</span>
                        <span className="font-semibold text-slate-900 dark:text-white font-mono">{formatCurrency(tax)}</span>
                      </div>
              ...
              <div className="bg-white dark:bg-[#0a192f] w-full rounded-sm shadow-md p-4 sm:p-6 md:p-12 min-h-[500px] md:min-h-[800px] text-slate-800 dark:text-slate-100 flex flex-col relative transition-all">
                {/* Header Facture */}
                <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-8 md:mb-12">
                  <div>
                    <h1 className="text-3xl md:text-4xl font-light text-slate-900 dark:text-white tracking-tight">FACTURE</h1>
                    <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1 font-mono">{invoiceNumber}</p>
                  </div>
                  <div className="text-left sm:text-right">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-white mt-2">{settings.name}</h3>
                    {settings.address.split('\n').map((line, idx) => (
                      <p key={idx} className="text-xs text-slate-500 dark:text-slate-400">{line}</p>
                    ))}
                    {settings.phone && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{settings.phone}</p>}
                    {settings.email && <p className="text-xs text-slate-500 dark:text-slate-400">{settings.email}</p>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-12">
                  <div className="order-2 sm:order-1">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-white">{selectedClient?.name || "Sélectionnez un client..."}</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{selectedClient?.email || "email@client.com"}</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{selectedClient?.phone || "N/A"}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4 order-1 sm:order-2">
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{issueDate ? new Date(issueDate).toLocaleDateString("fr-FR") : "-"}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Échéance</p>
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{dueDate ? new Date(dueDate).toLocaleDateString("fr-FR") : "-"}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b-2 border-slate-900">
                        <th className="text-left font-semibold text-slate-500 dark:text-slate-400 py-3">Description</th>
                        <th className="text-center font-semibold text-slate-500 dark:text-slate-400 py-3 w-16">Qté</th>
                        <th className="text-right font-semibold text-slate-500 dark:text-slate-400 py-3 w-28">Prix U.</th>
                        <th className="text-right font-semibold text-slate-500 dark:text-slate-400 py-3 w-32">Montant</th>
                      </tr>
                    </thead>
                    <tbody>
                      {lines.map((line, idx) => (
                        <tr key={idx} className="border-b border-slate-100 dark:border-[#152a4d] last:border-0">
                          <td className="py-4 text-slate-800 dark:text-slate-100 font-medium">{line.description || <span className="text-slate-300 italic">Article sans nom</span>}</td>
                          <td className="py-4 text-center text-slate-600 dark:text-slate-300">{line.quantity}</td>
                          <td className="py-4 text-right text-slate-600 dark:text-slate-300"><span className="font-mono">{formatCurrency(typeof line.unitPrice === 'number' ? line.unitPrice : (parseFloat(line.unitPrice) || 0))}</span></td>
                          <td className="py-4 text-right font-semibold text-slate-900 dark:text-white"><span className="font-mono">{formatCurrency((typeof line.quantity === 'number' ? line.quantity : (parseFloat(line.quantity) || 0)) * (typeof line.unitPrice === 'number' ? line.unitPrice : (parseFloat(line.unitPrice) || 0)))}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex justify-end mb-8 md:mb-12">
                  <div className="w-full sm:w-64 space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-slate-900 dark:text-white font-semibold"><span className="font-mono">{formatCurrency(subtotal)}</span></span>
                    </div>
                    {applyTax && (
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500 dark:text-slate-400 font-medium">TVA (18%)</span>
                        <span className="text-slate-900 dark:text-white font-semibold"><span className="font-mono">{formatCurrency(tax)}</span></span>
                      </div>
                    )}
                    <div className="flex justify-between text-lg pt-3 border-t border-slate-200 dark:border-[#1c3a66]">
                      <span className="font-bold text-slate-900 dark:text-white">Total TTC</span>
                      <span className="font-bold text-slate-900 dark:text-white"><span className="font-mono">{formatCurrency(total)}</span></span>
                    </div>
                  </div>
                </div>

                {/* Footer Facture */}
                <div className="mt-auto pt-8 border-t border-slate-100 dark:border-[#152a4d] flex justify-between items-end">
      ...
      {/* Success Modal */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm transition-opacity print:hidden">
          <div className="bg-white dark:bg-[#0a192f] rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden border border-slate-200 dark:border-[#1c3a66] transform scale-100 animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 text-center space-y-6">
              <div className="mx-auto w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4">
                <CheckCircle2 size={32} className="text-green-600" />
              </div>
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Facture enregistrée !</h2>
              ...
              <div className="flex flex-col gap-3 pt-2">
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="w-full border-slate-200 dark:border-[#1c3a66] text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:bg-[#06101e] py-6"
                >
