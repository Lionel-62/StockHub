...
export function AIAssistantButton() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (sessionStr) {
        ...
        const products = JSON.parse(localStorage.getItem(`stockhub_cache_products_${shopId}`) || '[]');
        const orders = JSON.parse(localStorage.getItem(`stockhub_cache_orders_${shopId}`) || '[]');
        const clients = JSON.parse(localStorage.getItem(`stockhub_cache_clients_${shopId}`) || '[]');
        ...
        setShopData({
          ...
          recentOrders: orders.slice(0, 5), // Seulement les 5 dernières pour économiser les tokens
          lowStockProducts: products.filter((p: any) => p.stock <= (p.alertThreshold || 5))
        });
      }
    } catch (e) {
  ...
  return (
    <>
      ...
      <div 
        className={`fixed bottom-6 right-6 z-50 w-[90vw] sm:w-[400px] bg-white rounded-3xl shadow-2xl overflow-hidden transition-all duration-300 origin-bottom-right flex flex-col ${isOpen ? 'scale-100 opacity-100 h-[600px] max-h-[80vh]' : 'scale-0 opacity-0 pointer-events-none h-0'}`}
      >
        {/* Header */}
            ...
            <div className="flex flex-col h-full">
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-70">
                ...
                <p className="text-sm text-slate-500 mt-2 px-4">
                  Je suis votre assistant intelligent. Je peux analyser vos ventes, vérifier votre stock et répondre à vos questions.
                </p>
              </div>
              <div className="mt-4">
            ...
            <>
              {messages.map((m, i) => (
                <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${m.role === 'user' ? 'bg-slate-200 text-slate-600' : 'bg-gradient-to-r from-[#0b213f] to-blue-800 text-white'}`}>
                  ...
                  <div className={`p-3 rounded-2xl max-w-[80%] text-sm ${
                    ...
                    {/* Un rendu markdown basique avec CSS whitespace-pre-wrap pour garder les retours à la ligne */}
        ...
        {/* Input */}
        <form onSubmit={handleSubmit} className="p-3 bg-white border-t border-slate-200 flex items-center gap-2 flex-shrink-0">
          <input 
            type="text" 
            value={input}
